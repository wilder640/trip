...
``` mermaid
graph TD
A == 車程時間 ==> B == 車程時間 ==> C  == 車程時間 ==> D == 車程時間 ==> E == 車程時間 ==> F
...
graph TD
A == 車程時間 ==>  B == 車程時間 ==> C == 車程時間 ==> D
B-. 車程時間 .->E:::optional -. 車程時間 .->C

A(JR九州Blossom那霸酒店)
B(沖繩世界)
C(SAN-A浦添西海岸PARCO CITY)
D(JR九州Blossom那霸酒店)
E(首里城)
