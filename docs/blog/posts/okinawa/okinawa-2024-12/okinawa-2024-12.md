...
``` mermaid
graph TD
    A(12:00<br>家) == 30分鐘 ==> B(12:30-15:30<br>高雄小港機場) == 1小時40分 ==> C(18:30-21:00<br>那霸機場) == 20分鐘 ==> E(21:30<br>Hotel Gran ViewGarden Okinawa)
```
...
``` mermaid
graph TD
    A(Hotel Gran ViewGarden Okinawa) == 車程時間 ==> B(OTS臨空豐崎營業所) == 車程時間 ==> C(波上宮)  == 車程時間 ==> D(港川外人住宅) == 車程時間 ==> E(沖繩兒童王國) == 車程時間 ==> 
    F(VESSEL HOTELCAMPANA OKINAWA) == 車程時間 ==> G(美國村)
```
...
``` mermaid
graph TD
  A(VESSEL HOTELCAMPANA OKINAWA) == 車程時間 ==> Ｂ(萬座毛) == 車程時間 ==> Ｃ(海洋博公園) == 車程時間 ==> Ｄ(沖繩餘家弗酒店)
```
...
``` mermaid
graph TD
  A(沖繩餘家弗酒店) == 車程時間 ==> B(名護鳳梨園) == 車程時間 ==>  C(古宇利大橋南端觀景台) == 車程時間 ==> D(古宇利蝦蝦飯) == 車程時間 ==> E(古宇利海洋塔) == 車程時間 ==> F(JR九州Blossom那霸酒店)
```
...
``` mermaid
graph TD
  A(JR九州Blossom那霸酒店) == 車程時間 ==> B(沖繩世界) == 車程時間 ==> C(SAN-A浦添西海岸PARCO CITY) == 車程時間 ==> D(JR九州Blossom那霸酒店)
```
...
``` mermaid
graph TD
  A(JR九州Blossom那霸酒店) == 車程時間 ==> B(國際通) == 車程時間 ==> C(那霸機場) == 車程時間 ==> D(高雄機場)
```
